...
public class ReservationServiceImpl implements ReservationService {
    ...
    public Reservation createPendingReservation(CreateBookingRequest request, Long vendorId) {
        ...
        // Validate that vendor has at most 3 active event reservations at the same time
        long activeEventsCount = reservationRepository.findByVendorOrderByBookingDatetimeDesc(vendor).stream()
                .filter(r -> r.getStatus() == ReservationStatus.PENDING || r.getStatus() == ReservationStatus.SUCCESS)
                .filter(r -> r.getEvent().getEventDate().isAfter(LocalDateTime.now()))
                .map(r -> r.getEvent().getId())
                .distinct()
                .count();
        if (activeEventsCount >= 3) {
            throw new IllegalStateException("Cannot book more than 3 events at the same time.");
        }

        List<Stall> stalls = stallRepository.findAllByIdForUpdate(request.getStallIds());
        ...
        for (Stall s : stalls) {
            if (alreadyBooked.contains(s.getId())) {
                throw new IllegalStateException("One or more selected stalls are no longer available.");
            }
        }
        ...
        reservation.setNumberOfStallsRequired(request.getNumberOfStallsRequired());
        ...
        reservation.setPaymentMethod(request.getPaymentMethod()); // Enum validation handled by
    ...
    public void approveReservation(Long reservationId) {
        ...
        if (reservation.getStatus() != ReservationStatus.PENDING) {
            throw new IllegalStateException("Reservation is not in PENDING state (current: " + reservation.getStatus() + ")");
        }
    ...
    public void cancelReservation(Long reservationId, Long vendorId) {
        ...
        if (reservation.getStatus() != ReservationStatus.PENDING && reservation.getStatus() != ReservationStatus.SUCCESS) {
