...
export default function Booking() {
  ...
  const [preferredStallSize, setPreferredStallSize] = useState('MEDIUM');
  const [businessCategory, setBusinessCategory] = useState('SERVICES');
  ...
  useEffect(() => {
    ...
    Promise.all([eventsApi.get(id), publicApi.genres()])
      .then(([ev, g]) => {
        ...
        if (ev.eventDate) {
          setReservationDate(ev.eventDate.substring(0, 10));
        }
      })
      .catch(() => setEvent(null))
  ...
  const handleSubmit = (e) => {
    ...
    navigate('/payment/new', {
      state: {
        ...
        // OIDC Validation fields
        stallType,
        preferredStallSize,
        numberOfStallsRequired: selectedStallIds.length,
        businessCategory,
        reservationDate: reservationDate || event.eventDate.substring(0, 10),
        specialRequirements
      }
    });
  };
    ...
    <div className="min-h-screen bg-gray-900 py-12 px-4 text-gray-100">
      <div className="container mx-auto max-w-5xl space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* OIDC Reservation Details Form Fields */}
          <div className="bg-slate-800 border border-stone-700 rounded-xl p-6 space-y-4 shadow-md">
            <h3 className="text-xl font-semibold text-blue-300">Reservation Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-stone-300 mb-1">Stall Type</label>
                <select
                  value={stallType}
                  onChange={(e) => setStallType(e.target.value)}
                  className="w-full bg-slate-700 border border-stone-600 rounded-lg p-2 text-white focus:ring-amber-500 focus:border-amber-500"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-stone-300 mb-1">Preferred Stall Size</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-stone-300 mb-1">Business Category</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-stone-300 mb-1">Reservation Date</label>
                <input
                  type="date"
            ...
            <div>
              <label className="block text-sm font-medium text-stone-300 mb-1">Special Requirements / Comments (Max 500 chars)</label>
              <textarea
